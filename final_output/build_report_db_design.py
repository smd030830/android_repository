...
DOCX_PATH = OUT / '최종보고서_DogLog_DB설계반영.docx'
FONT = '맑은 고딕'
...
p(doc, 'DB 테이블 캡처본 기준 실제 저장 레코드', 'Heading 2')
p(doc, '아래 표는 제출 직전 DBMS 화면에서 확인한 테이블 캡처본을 기준으로 정리한 실제 저장 데이터이다. 회원가입, 강아지 등록, 일지 작성, 문의/답장 기능이 DB 테이블에 반영되었음을 보여준다.', size=9)
p(doc, 'USER 테이블', 'Heading 3')
p(doc, '기존 JSP 호환 로그인 테이블이다. 캡처본에는 임시보호자 계정과 입양희망자 계정이 각각 저장되어 있다.', size=9)
...
p(doc, 'APP_USER 테이블', 'Heading 3')
p(doc, '신규 회원가입 저장용 사용자 테이블이다. 임시보호자와 입양희망자가 함께 저장되어 로그인 후 사용자 유형별 화면 분기에 사용된다.', size=9)
table(doc, ['userID', 'userPassword', 'userEmail', 'userGender', 'userType'], [
...
p(doc, 'DOG 테이블', 'Heading 3')
p(doc, '강아지 등록 결과가 저장된 테이블이다. 캡처본의 마루, 밀크, 호두는 모두 fosterId가 test123으로 저장되어 있어 임시보호자 본인 강아지 목록 필터링과 일지 작성 권한 검증에 사용된다. photoData는 Base64 이미지 데이터이므로 일부만 표기하였다.', size=9)
...
p(doc, 'DIARY 테이블', 'Heading 3')
p(doc, '일지 작성 결과가 저장된 테이블이다. 캡처본에서 한국어 본문, 사료량(foodAmount), 배변량(poopCount), 사진 데이터(photoData), 작성자 fosterId가 함께 저장된 것을 확인할 수 있다.', size=9)
...
p(doc, 'CONTACT_MESSAGE 테이블', 'Heading 3')
p(doc, '입양희망자의 문의와 임시보호자의 답장이 저장된 테이블이다. senderId는 문의 작성자, receiverId는 임시보호자이며 replyContent와 repliedAt으로 답장 여부를 확인한다.', size=9)
...
p(doc, '캡처본 기준 CRUD 확인', 'Heading 3')
bullets(doc, [
